...
describe('api-admin skeleton (e2e)', () => {
  ...
  it('registers SharedModule with AUDIT_APP=ADMIN, not any other app', () => {
    expect(app.get(AUDIT_APP)).toBe('ADMIN');
  });

  it('agrees on the request id across the response header, CLS and the audit row it writes', async () => {
    const res = await request(app.getHttpServer())
      .get('/api/probe/audit')
      .set('User-Agent', 'app-module-probe/api-admin')
      .expect(200);
    ...
    const row = await app.get(PrismaService).auditLog.findFirst({
      where: { action: 'auth.invite.issued', userAgent: 'app-module-probe/api-admin' },
    });
    expect(row).toMatchObject({
      app: 'ADMIN',
      action: 'auth.invite.issued',
      outcome: 'SUCCESS',
      userAgent: 'app-module-probe/api-admin',
    });
  });
});
