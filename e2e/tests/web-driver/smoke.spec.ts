...
test.describe('web-driver smoke', () => {
  test('serves the SPA on the kiosk origin', async ({ page }) => {
    await page.goto('/');
    await expect(page).toHaveTitle('TMS Kiosk');
    await expect(page.getByTestId('app-root')).toBeAttached();
  });

  test('forwards /api to api-driver on the same origin (D11)', async ({ request }) => {
